...
class Session implements SessionInterface
{
    ...
    {
        if (!$this->started) {
            throw new Exceptions\SessionError('Session has not been started');
        }
    ...
    public function cookie(): string
    {
        ...
        if ($expire = $this->getOption('expire')) {
            $gmdate = (new DateTimeImmutable())->setTimezone(new DateTimeZone('GMT'))
                ->add(new DateInterval(sprintf('PT%uS', $expire)));
            $pairs[] = sprintf(
                'Expires=%s; Max-Age=%u',
